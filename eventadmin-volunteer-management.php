...
define('EVENTADMIN_VERSION', '3.6.0');
...
require_once plugin_dir_path(__FILE__) . 'includes/open-positions.php';
...
function eventadmin_update_notice(): void
{
    ...
    <div id="eventadmin-update-notice" style="clear:both;background:#fff;border-left:4px solid #2271b1;padding:16px 20px;margin:20px 20px 20px 0;box-shadow:0 1px 4px rgba(0,0,0,.08);display:flex;align-items:flex-start;gap:16px;">
        ...
        <div style="flex:1;">
            ...
            <?php echo esc_html__('New in this release: the MCP tools can now see who is assigned to a shift and email that shift\'s roster directly, plus new tools to look up departments, sign-up rules, and the plugin\'s own documentation. Also fixes a clipboard-copy bug on non-HTTPS sites and a CSV import bug affecting accented or Nordic letters.', 'eventadmin-volunteer-management'); ?>
 ...
 * @return string[]
 */
function eventadmin_plugin_action_links(array $links): array
{
    array_unshift(
        $links,
...
add_filter('plugin_action_links_' . plugin_basename(__FILE__), 'eventadmin_plugin_action_links');
